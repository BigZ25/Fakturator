...
RUN apt-get update && apt-get install -y \
    ...
    && docker-php-ext-install -j$(nproc) gd pdo pdo_mysql zip soap
...
RUN echo "<VirtualHost *:80>\n\
    ...
    ServerName fakturator.localhost\n\
    ServerAlias *.fakturator.localhost\n\
...
</VirtualHost>" >> /etc/apache2/sites-available/fakturator.conf

# Enable the virtual host
RUN a2ensite fakturator
